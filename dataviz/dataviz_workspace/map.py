...
def create_document(title, description=''):
    ...
    # Pull in common attributes from www.opengis.net and set it for our doc
    kml.setAttribute('xmlns','http://www.opengis.net/kml/2.2')
    doc.appendChild(kml)

    # Create common metadata Title and Description elements that Google will read/plot
    document = doc.createElement('Document')    #<document></document>
    kml.appendChild(document)
    docName = doc.createElement('title')   #<title></title>
...
#my guess--we take our overall document and then
#for each of the placemarks from this function, we
#append them as children to the overall document from the
#create_document function.  Append after the <Document>Node
def create_placemark(address):
    ...
    pm = doc.createElement('Placemark')    #<Placemark><Name>name_text</Name><description></description></Placemark>
    doc.appendChild(pm)     
...
def create_gmap(data_file):
    ...
    list_of_document_elems = kml_doc.getElementsByTagName("Document")
    # Returns a list, so call the first one
    document_node = list_of_document_elems[0]
    # Iterate over our list of dicts from csv_data to create KML document
    for line in data_file:
        ...
        kml_form = create_placemark(placemark_info)

        # Adds the placemark we just created to the KML doc
        document_node.appendChild(kml_form.documentElement)
...
#here's what our csv data looksl ike
