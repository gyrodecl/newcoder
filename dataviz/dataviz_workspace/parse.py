...
def myparse(raw_file, delimiter):
    """Parses a raw CSV file toa JSON-line object."""
    opened_file = open(raw_file, 'rb')
    csv_data = csv.DictReader(opened_file)
    parsed_data = []
    for line in csv_data:   #each line will be a dict with all the key:value pairs 
...
def parse(raw_file, delimiter):
    ...
    for row in csv_data:
        parsed_data.append(dict(zip(fields, row)))  #zip gives list of tuples [('category','drug'),('incidentnum','030206159')]
    # Close the CSV file
    opened_file.close()
